...
/**
 * \brief 为新生成的 ASS_Image 链表节点设置 channel_images
 * \param first_new 第一个新添加的 ASS_Image（render_glyph 调用前 *old_tail 为 NULL，调用后为第一个新节点）
...
static ASS_Image *render_text(RenderContext *state)
{
    ...
    for (unsigned i = 0; i < n_bitmaps; i++) {
        // VSFilterMod: 释放 channel_images 字符串
        for (int ci = 0; ci < 4; ci++) {
            free(bitmaps[i].channel_images[ci]);
            bitmaps[i].channel_images[ci] = NULL;
        }
...
void ass_reset_render_context(RenderContext *state, ASS_Style *style)
{
    ...
    // VSFilterMod: 清除图片填充路径
    for (int i = 0; i < 4; i++) {
        state->channel_images[i].str = NULL;
        state->channel_images[i].len = 0;
    }
}
...
static bool parse_events(RenderContext *state, ASS_Event *event)
{
    ...
    while (true) {
        ...
        // VSFilterMod: 复制图片填充路径到 GlyphInfo
        for (int ci = 0; ci < 4; ci++) {
            info->channel_images[ci] = state->channel_images[ci];
        }
            ...
            if (new_run) {
                ...
                for (int ci = 0; ci < 4; ci++) {
                    if (info->channel_images[ci].str && info->channel_images[ci].len > 0) {
                        ...
                        if (current_info->channel_images[ci]) {
                            memcpy(current_info->channel_images[ci],
                                   info->channel_images[ci].str,
                                   info->channel_images[ci].len);
                            current_info->channel_images[ci][info->channel_images[ci].len] = '\0';
                        }
                    } else {
                        current_info->channel_images[ci] = NULL;
                    }
                }
...
void ass_frame_unref(ASS_Image *img)
{
    ...
    do {
        ...
        img = img->next;
        // VSFilterMod: 释放 channel_images 字符串副本
        for (int i = 0; i < 4; i++) {
