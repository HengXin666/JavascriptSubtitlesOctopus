...
#define complex_tag(name) mystrcmp(&p, (name))
        ...
        // VSFilterMod: \1img(path), \2img(path), \3img(path), \4img(path)
        // 图片填充标签, 将图片作为颜色通道的纹理
        } else if (complex_tag("1img")) {
            if (nargs) {
                state->channel_images[0].str = args[0].start;
                state->channel_images[0].len = args[0].end - args[0].start;
            } else {
                state->channel_images[0].str = NULL;
                state->channel_images[0].len = 0;
            }
        } else if (complex_tag("2img")) {
            if (nargs) {
                state->channel_images[1].str = args[0].start;
                state->channel_images[1].len = args[0].end - args[0].start;
            } else {
                state->channel_images[1].str = NULL;
                state->channel_images[1].len = 0;
            }
        } else if (complex_tag("3img")) {
            if (nargs) {
                state->channel_images[2].str = args[0].start;
                state->channel_images[2].len = args[0].end - args[0].start;
            } else {
                state->channel_images[2].str = NULL;
                state->channel_images[2].len = 0;
            }
        } else if (complex_tag("4img")) {
            if (nargs) {
                state->channel_images[3].str = args[0].start;
                state->channel_images[3].len = args[0].end - args[0].start;
            } else {
                state->channel_images[3].str = NULL;
                state->channel_images[3].len = 0;
            }
        }
    }
